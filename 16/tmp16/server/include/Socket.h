...
private:
    const int fd_;
    std::string servip_;
    uint16_t servport_=-1;
    std::string clientip_;
    uint16_t clientport_=-1;
public:
    Socket(const int fd);
    Socket(const int fd,const std::string& clientip, uint16_t clientport);
    void closefd();
    ~Socket();

    const int fd() const;
    const std::string servip() const;
    const uint16_t servport() const;
