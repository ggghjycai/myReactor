...
多线程的主从reactor模型:
      处理客户端连接的主线程 (一个)
      处理客户端数据  收发(只收发,不处理数据)    的I/O线程 (多个)
      处理客户端数据 处理I/O线程发送过来的数据(只处理业务,不发送) 工作线程(多个)


使用智能指针:
要点:  指针在哪个对象的析构函数中销毁, 那么智能指针就属于哪个对象 (也就是属于控制其生命周期的对象)
shared_ptr:
Connection* 必须使用智能指针shared_ptr,目的是为了解决 I/O线程 接收到客户端断开连接的消息立即销毁对象导致 工作线程使用了野指针
unique_ptr:  注意要使用移动语义
其他的指针可以使用裸指针, 也可以使用unique_ptr.  当然,如果开销比较小,甚至可以在栈上创建就行.
使用智能指针的主要问题:
  在回调函数中绑定的时候 ,关注是否要使用.get()方法返回裸指针


//使用初始化列表注意事项:
  对于类对象，成员变量在构造函数体执行前就已被初始化。**如前所述，初始化顺序由类内部的声明顺序确定，而不是初始化列表中的顺序。
...
int main(int argc, char *argv[])
{
  if (argc != 8)
  {
    printf("usage:./echoServer ip port IO_THREAD WORK_THREAD LOOP_TIMEVAL CONNECTION_TIMEOUT ACCEPTTYPE\n");
    printf("example:./echoServer 192.168.126.200 5055 3 3 10 5 1\n\n");
    return -1;
  }
  ...
  echoserver=new EchoServer(argv[1],atoi(argv[2]),atoi(argv[3]),atoi(argv[4]),atoi(argv[5]),atoi(argv[6]),atoi(argv[7]));
  echoserver->Start();
}