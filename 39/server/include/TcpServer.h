...
private:
    ...
    std::function<void(spConnection)> newconnectioncb_;          // 回调上层业务::HandleNewConnection()。
    std::function<void(spConnection)> closeconnectioncb_;        // 回调上层业务::HandleClose()。
    std::function<void(spConnection)> errorconnectioncb_;         // 回调上层业务::HandleError()。
    std::function<void(spConnection,std::string &message)> onmessagecb_;        // 回调上层业务::HandleMessage()。
    std::function<void(spConnection)> sendcompletecb_;            // 回调上层业务::HandleSendComplete()。
    std::function<void(EventLoop*)>  timeoutcb_;                       // 回调上层业务::HandleTimeOut()。
