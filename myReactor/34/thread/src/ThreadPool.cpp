    ...
    for (size_t ii = 0; ii < threadnum; ii++)
    {
        ...
        ([this]{          
            ...
            while(stop_==false)
            {
                ...
                //printf("thread is %ld.\n",syscall(SYS_gettid));
                printf("%s(%ld) execute %s.\n",threadtype_.c_str(),syscall(SYS_gettid),this->function_name_.c_str());
                //接下来执行任务:
                task();
            }   
        });
...
void ThreadPool::addtask(std::function<void()> task,const std::string& function_name)
{   
  ...
  const uint32_t ThreadPool::size()
  {
...
ThreadPool::~ThreadPool()
{
    ...
    for(std::thread &val:threadpool_)
    {
        val.join();
    }
}
