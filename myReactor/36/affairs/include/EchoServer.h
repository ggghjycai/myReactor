...
public:
    EchoServer(const std::string &ip,const uint16_t port,int subthreadnum=3,
            int workthreadnum=5,int wroktimevaL=5,int worktimeout=10,int accepttype=1);
    ~EchoServer();
    // 启动服务。
    void Start();    
    //终止服务
...
private:
    int checkAcceptType(int accepttype);
};