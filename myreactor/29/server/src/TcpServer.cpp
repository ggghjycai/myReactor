...
TcpServer::TcpServer(const std::string &ip, const uint16_t port,int threadnum):threadnum_(threadnum)
{
    ...
    for(int ii=0;ii<threadnum_;ii++)
    {
        ...
        threadpool_->addtask(std::bind(&EventLoop::run,subloops_[ii]),"EventLoop::run()");  // 在线程池中运行从事件循环。
    }
...
void TcpServer::newconnection(Socket *clientsock)
{
    ...
    spConnection conn=std::make_shared<Connection>(subloops_[clientsock->fd()%threadnum_], clientsock);   // C++11标准，效率更高。
