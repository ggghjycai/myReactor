...
void Socket::setSO_REUSEADDR(bool flag)
{
    ...
    {
        int opt = 1;
        ::setsockopt(fd_, SOL_SOCKET, SO_REUSEADDR, &opt,sizeof(opt) /* sizeof(static_cast<socklen_t>(opt)) */);
    }
}
...
void Socket::setTCP_NODELAY(bool flag)
{
    ...
    {
        int opt = 1;
        ::setsockopt(fd_, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)/* sizeof(static_cast<socklen_t>(opt) */);
    }
}
...
void Socket::setSO_REUSEPORT(bool flag)
{
    ...
    {
        int opt = 1;
        ::setsockopt(fd_, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)/* sizeof(static_cast<socklen_t>(opt)) */);
    }
}
...
void Socket::setSO_KEEPALIVE(bool flag)
{
    ...
    {
        int opt = 1;
        ::setsockopt(fd_, SOL_SOCKET, SO_KEEPALIVE, &opt, sizeof(opt)/* sizeof(static_cast<socklen_t>(opt)) */);
    }
}
...
const uint16_t Socket::port() const
{
    return port_;
}
