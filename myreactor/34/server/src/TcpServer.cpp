...
// TCP网络服务类
TcpServer::TcpServer(const std::string &ip, const uint16_t port,int threadnum,int timeval,int timeout)  
                    :threadnum_(threadnum),mainloop_(new EventLoop(true,timeval,timeout)),acceptor_(mainloop_.get(),ip,port),
                    threadpool_(threadnum_,"IO"),tcpcas_(new CASL())
{
...
void TcpServer::newconnection(std::unique_ptr<Socket> clientsock)
{
    ...
    int random=clientsock->fd()%threadnum_;
                                                                                    //.get()超级细节
    spConnection conn=std::make_shared<Connection>(subloops_[random].get(), std::move(clientsock));   // C++11标准，效率更高。
