...
BankServer::BankServer(const std::string &ip,const uint16_t port,int subthreadnum,
                        int workthreadnum,int wroktimevaL,int worktimeout,int accepttype)
:tcpserver_(ip,port,subthreadnum,wroktimevaL,worktimeout,checkAcceptType(accepttype)),threadpool_(workthreadnum,"WORKS")    //初始化tcpserver_和 工作线程threadpool_
{
...
bool getxmlbuffer(const std::string &xmlbuffer,const std::string &fieldname,std::string  &value,const int ilen=0)
{
 ...
 void BankServer::OnMessage(spConnection conn,std::string& message)     
 {
    ...
    if (bizcode=="00101")     // 登录业务。
    {
        ...
        if ( (username=="cz") && (password=="123465") )         // 假设从数据库或Redis中查询用户名和密码。
        {
            // 用户名和密码正确。
...
int BankServer::checkAcceptType(int accepttype) {
    // 检查accepttype是否合法，如果不合法可以抛出异常或返回一个合法的值
    if (accepttype<0||accepttype>2) {
     printf("暂不接受该种格式,系统改为默认的报文头处理: 1\n"); 
     accepttype=1;
    }
    return accepttype;
}